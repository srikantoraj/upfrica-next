...
import { setUser as setAuthUser } from "@/app/store/slices/userSlice";
...
export default function LoginForm() {
  ...
  useEffect(() => {
    console.log("🟣 LoginForm mounted (client persists token to Redux/localStorage)");
  }, []);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        // ⬇️ AuthContext.login() should return { user, token, onboarding }
        const payload = await login(values.email.trim(), values.password, !!values.rememberMe);
        const { user, token, onboarding } = payload || {};

        // ✅ Persist to Redux + localStorage
        dispatch(setAuthUser({ user: user || null, token: token || null }));

        // Redirect logic
        const hasAccountType = Array.isArray(user?.account_type) && user.account_type.length > 0;

        let target = onboarding?.target || "/new-dashboard";
        ...
        <form onSubmit={formik.handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            <button
              ...
              className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md bg-purple-600 text-white font-medium hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-60"
            >
              {formik.isSubmitting ? (
                <svg
