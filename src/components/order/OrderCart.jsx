                    ...
                    product?.cart_items?.length > 0 && (
                        ...
                        >
                            <div className="text-base">
                                ...
                                <p className="text-gray-600">Order Date: {product.created_at || 'N/A'}</p>
                            ...
                            <div className="grid md:grid-cols-2 py-5">
                                <div className="col-span-1 flex items-center">
                                    <img
                                        src={
                                            product.cart_items[0].product?.product_images?.[0] || '/default-image.jpg'
                                        }
                                        alt={product.cart_items[0]?.product?.title || 'No Title'}
                                        className="w-20 h-20 object-cover rounded-md"
                                    />
                                    <div className="ml-4 text-base font-bold">
                                        <p className="text-gray-700">{product.cart_items[0]?.product?.title || 'No Title'}</p>
                                        <p className="text-gray-600">Quantity: {product.cart_items[0].quantity || 0}</p>
                                        <p className="text-gray-600">{product.is_paid ? 'Paid' : 'Unpaid'}</p>
                                        <p className="text-gray-600">Dispatch time: {product.dispatch_time || 'N/A'}</p>
                                        <p className="text-gray-600">
                                            Price: ${product.cart_items[0].product?.price?.cents ? (product.cart_items[0].product.price.cents / 100).toFixed(2) : 'N/A'}
                                        </p>
                                        <p className="text-gray-600">Postage & Packing: {product.postage_packing || 'N/A'}</p>
                                        <p className="text-gray-600">
                                ...
                                <div className="col-span-1 flex justify-center items-center">
                                    <div className="grid grid-cols-4 gap-2">
                                        <button className="text-black px-4 py-1 border rounded hover:border-red-400">Checkout</button>
                                        <button
                            ...
                            <div className="p-4 bg-white rounded">
                                ...
                                <p className="text-gray-700">{product?.buyer?.phone_number}</p>
