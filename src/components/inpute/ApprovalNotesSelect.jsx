...
const ApprovalNotesSelect = ({ formik }) => {
  ...
  const OPTIONS = [
    { value: "", label: "Select" },
    { value: "Phone number or contact details not allowed on images", label: "Phone number or contact details not allowed on images" },
    { value: "Please remove text from the main image", label: "Please remove text from the main image" },
    { value: "Please upload a clearer image", label: "Please upload a clearer image" },
    { value: "Provide a valid contact number. Go to your Profile Settings", label: "Provide a valid contact number. Go to your Profile Settings" },
    { value: "Description should be in bullet points, include keywords", label: "Description should be in bullet points, include keywords" },
    { value: "The first 2-3 words in the title must contain main keywords", label: "The first 2-3 words in the title must contain main keywords" },
    { value: "Missing Supplier Link", label: "Missing Supplier Link" },
    { value: "Missing Supplier phone number", label: "Missing Supplier phone number" },
  ...
  return (
    <div className="bg-white shadow rounded-lg mb-6">
      <div className="p-4 space-y-4">
        {/* dropdown */}
        <div>
          <label htmlFor="approval_notes" className="block text-lg font-semibold mb-2">
            Approval notes
          </label>
          <select
            id="approval_notes"
          ...
          {formik.touched.approval_notes && formik.errors.approval_notes ? (
            <p className="text-red-500 text-sm mt-1">{formik.errors.approval_notes}</p>
          ) : null}
        </div>
        ...
        {formik.values.approval_notes === "Other" && (
          <div>
            <label htmlFor="approval_notes_custom" className="block text-lg font-semibold mb-2">
            ...
            <textarea
              ...
              rows={4}
            />
            {formik.touched.approval_notes_custom && formik.errors.approval_notes_custom ? (
              <p className="text-red-500 text-sm mt-1">{formik.errors.approval_notes_custom}</p>
            ) : null}
          </div>
        )}
      </div>
