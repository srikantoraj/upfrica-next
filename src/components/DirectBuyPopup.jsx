...
//     console.log("DirectBuyPopup related data ", relatedProducts);
...
//     // ✅ When modal opens, set initial quantity
//     useEffect(() => {
//         if (isVisible) {
...
//         >
//             <div
//                 className="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4 p-4 md:p-6 relative"
...
//                                             });
//                                             setDirectBuyQuantity(1); // ✅ Reset quantity on image click
//                                         }}
//                                     >
//                                         {/* Image */}
...
//                                         {/* Title */}
//                                         <p className="mt-1 text-xs font-medium text-gray-700 truncate" title={item.title}>
//                                             {item.title}
//                                         </p>
...
const DirectBuyPopup = ({ product, isVisible, onClose, quantity, relatedProducts }) => {
  ...
  const estimatedDelivery = (() => {
    if (product?.dispatch_time_in_days) {
      const deliveryDate = new Date();
      deliveryDate.setDate(deliveryDate.getDate() + parseInt(product.dispatch_time_in_days));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4 p-4 md:p-6 relative" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center border-b pb-3">
          <h3 className="text-xl font-semibold">Buy Now</h3>
          <button onClick={onClose} className="text-gray-600 hover:text-gray-900"><HiXMark className="h-6 w-6" /></button>
        </div>

        <div className="mt-4 space-y-4">
          <div className="flex items-center gap-4 p-3 border rounded">
            <img src={selectedProduct.image || "https://via.placeholder.com/80"} className="w-[100px] h-[100px] object-cover rounded" />
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-800 line-clamp-2">{selectedProduct.title}</p>
              <p className="text-sm text-gray-600">₵{(selectedProduct.price / 100).toFixed(2)}</p>
              <div className="flex items-center mt-2">
                <button onClick={decrementQuantity} className="px-2 py-1 border rounded-l">–</button>
                <div className="px-4 py-1 border-t border-b font-medium">{directBuyQuantity}</div>
                <button onClick={incrementQuantity} className="px-2 py-1 border rounded-r">+</button>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1 text-sm text-gray-700">
            ...
            <p>Delivery Charges: <span className="font-medium">{product?.postage_fee_cents ? `₵${(product.postage_fee_cents / 100).toFixed(2)}` : "Free"}</span></p>
          </div>
          ...
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">Payment Method</p>
            <div className="flex gap-6">
              <label className="flex items-center"><input type="radio" value="stripe" checked={paymentMethod === "stripe"} onChange={() => setPaymentMethod("stripe")} className="mr-2" /> Stripe</label>
              <label className="flex items-center"><input type="radio" value="paystack" checked={paymentMethod === "paystack"} onChange={() => setPaymentMethod("paystack")} className="mr-2" /> Paystack</label>
            </div>
          </div>

          <div><p className="text-sm font-medium text-gray-700">Total Charges: ₵{((selectedProduct.price / 100) * directBuyQuantity + (product?.postage_fee_cents || 0) / 100).toFixed(2)}</p></div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Shipping Address</label>
            {isAddressLoading ? (
              <div className="animate-pulse"><div className="h-10 bg-gray-300 rounded w-full" /></div>
            ) : (
        ...
        {relatedProducts && relatedProducts.length > 0 && (
          <div className="mt-6">
            <p className="text-sm font-medium text-gray-700 mb-2">Related Products</p>
            <div className="flex overflow-x-auto space-x-3 pb-2">
              {relatedProducts.filter(p => p.image_url).slice(0, 10).map(item => (
                <div
                  key={item.id}
                  className="flex-shrink-0 w-[100px] cursor-pointer hover:opacity-90 transition text-center"
                  onClick={() => {
                ...
                >
                  ...
                  <p className="mt-1 text-xs font-medium text-gray-700 truncate">{item.title}</p>
                  <p className="text-[11px] text-gray-800 font-semibold">₵{((item.sale_price_cents || item.price_cents) / 100).toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
