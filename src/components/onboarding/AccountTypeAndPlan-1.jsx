...
export default function AccountTypeAndPlan() {
  ...
  const sellerSelected = selectedRoles.includes('seller_private') || selectedRoles.includes('seller_business');

  useEffect(() => {
    if (token) {
      axiosInstance.get('/api/seller-plans/', {
                  headers: {
  ...
  const toggleRole = (value) => {
    ...
    const hasBothSellers = updated.includes('seller_private') && updated.includes('seller_business');
    const hasSellerAndAgent = (updated.includes('seller_private') || updated.includes('seller_business')) && updated.includes('agent');
  ...
  const handleSubmit = async () => {
    ...
    const isFreePlan = !selectedPlan || Number((plans.find(p => p.id === selectedPlan)?.price_per_month || 0)) === 0;
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <p className="text-gray-500 mb-6">Choose what you want to do on Upfrica.</p>
...
<div className="flex items-center gap-4 mb-4">
  ...
  <span className={`text-sm font-medium ${billingCycle === 'weekly' ? 'text-black' : 'text-gray-500'}`}>
    Weekly
  </span>
</div>
            ...
            {filteredPlans.map(plan => (
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  {selectedPlan === plan.id && <span className="text-green-600">✔ Selected</span>}
                </div>
              </button>
            ))}
          </div>
