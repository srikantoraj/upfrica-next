...
function pickRawImage(r) {
  ...
  const arrays = [r.image_objects, r.product_images, r.images].filter(Array.isArray);
  ...
  return first([
    r.image, r.image_url, r.thumbnail, r.thumbnail_url,
...
// Minimal gate respects STRICT toggles
function passesMinimal(n) {
  if (STRICT.requireTitle && !truthyStr(n.title)) return false;
  if (STRICT.requireImage && !truthyStr(n.image)) return false;
  if (STRICT.requirePrice && !(Number.isFinite(n.price) && n.price > 0)) return false;
  return true;
}
function passesStrictRaw(n) {
  const r = n._raw;
  if (!r) return true;
  if (STRICT.requireCondition && !hasConditionRaw(r)) return false;
  if (STRICT.requireCategory  && !hasCategoryRaw(r))  return false;
  if (STRICT.requireImage     && !hasImageRaw(r))     return false;
  if (STRICT.requirePrice     && !hasPriceRaw(r))     return false;
  return true;
}
function frontpageReady(n) {
  return passesMinimal(n) && passesStrictRaw(n);
}
...
export default async function ProductRail({
  ...
  // Prefer provided items, but if it's an empty array, FALL BACK to list API
  ...
  // Normalize and filter (respecting STRICT toggles)
  const normalized = (sourceItems || []).map((it) => normalizeItem(it, cc));
  const safeItems  = normalized.filter(frontpageReady);
