...
async function presignProductUpload(productId, file) {
  const resp = await fetch("/api/uploads/presign/", {
    ...
    body: JSON.stringify({
      kind: "product",
      product_id: productId,
      filename: file.name, // use compressed/renamed filename
      contentType: file.type || "application/octet-stream",
      content_type: file.type || "application/octet-stream",
    }),
  });
...
async function compressAtLeastHalf(file) {
  // fast pass: WebP @ 0.72, max 1600px
  const tryCompress = async (quality, maxDim, type) =>
    imageCompression(file, {
      maxWidthOrHeight: maxDim,
      initialQuality: quality,
      fileType: type,
      useWebWorker: true,
      convertSize: Infinity, // always convert
    });

  const t1 = await tryCompress(0.72, 1600, "image/webp");
  if (t1.size <= file.size * 0.5) return new File([t1], replaceExt(file.name, ".webp"), { type: "image/webp" });

  const t2 = await tryCompress(0.62, 1400, "image/webp");
  if (t2.size <= file.size * 0.5) return new File([t2], replaceExt(file.name, ".webp"), { type: "image/webp" });

  const t3 = await tryCompress(0.52, 1200, "image/webp");
  if (t3.size <= file.size * 0.5) return new File([t3], replaceExt(file.name, ".webp"), { type: "image/webp" });

  // fallback: JPEG – sometimes smaller for noisy photos
  const tj = await tryCompress(0.6, 1200, "image/jpeg");
  return new File([tj], replaceExt(file.name, ".jpg"), { type: "image/jpeg" });
}
...
export default function PhotosSection({
  seoTitle,        // ✅ NEW: used for SEO filenames
  productId,
  ensureProductId,
  onCountChange,
  onUploaded,
  ...
  // ---------- MAIN: pick → compress → preview → presign → upload → attach ----------
  async function onPickImages(e) {
    setError("");
    const rawFiles = Array.from(e.target.files || []).filter((f) => /^image\//i.test(f.type));
    e.target.value = "";
    ...
    try {
      ...
      // compress all first, build SEO names
      const base = slugify(seoTitle || "image");
      const compressed = [];
      for (let i = 0; i < rawFiles.length; i += 1) {
        const f = rawFiles[i];
        let c;
        try {
          c = await compressAtLeastHalf(f);
        } catch {
          c = f; // fallback: use original if compression fails
        }
        const ext = c.type === "image/webp" ? ".webp" : c.type === "image/jpeg" ? ".jpg" : (/\.[^.]+$/.exec(c.name)?.[0] || ".jpg");
        const renamed = new File([c], `${base}-${i + 1}${ext}`, { type: c.type });
        compressed.push(renamed);
      }

      // optimistic previews from compressed files (not originals)
      const startLen = images.length;
      const tempEntries = compressed.slice(0, 10).map((f, idx) => ({
        id: Date.now() + Math.random(),
        url: URL.createObjectURL(f),
        primary: startLen === 0 && idx === 0,
        _temp: true,
      ...
      for (let i = 0; i < compressed.length; i += 1) {
        ...
        const sig = await presignProductUpload(id, file);
        const publicUrl = await uploadWithSignature(sig, file);
        const saved = await attachImageUrl(id, publicUrl);
        const savedUrl = saved.image_url || saved.external_url || saved.image || "";
