...
// dynamic-import your Client UI, with SSR off
const EarlyDealsClient = dynamic(
    () => import('./EarlyDealsClient'),
    { ssr: false }
)
...
const EarlyDeals = async () => {
    ...
    try {
        ...
        return (
            <div className="container mx-auto p-4">
                <h1 className="text-xl font-bold text-red-600">Error parsing product data</h1>
                <p className="text-base">{err.message}</p>
            </div>
        )
    }

    // hand off to the Client Component
    return <EarlyDealsClient products={productsData.results} />
}
