    ...
    axiosInstance.get('/api/seller-plans/')
      .then((res) => {
        ...
        rawPlans.forEach(plan =>
          plan.feature_matrix?.forEach(fm =>
            fm.feature?.name && allFeaturesSet.add(fm.feature.name)
          )
        );
        ...
        const normalizedPlans = rawPlans.map(plan => {
          ...
          allFeatures.forEach(f => {
            featureMap[f] = plan.feature_matrix?.some(fm => fm.feature?.name === f && fm.enabled) || false;
          });
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl">
        <DialogTitle className="text-xl font-bold mb-4">Compare Seller Plans</DialogTitle>
                ...
                {plans.map((plan) => (
                  <th key={plan.id} className="text-center p-2 border-b">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs text-muted-foreground">GHS {plan.price}</span>
                      {plan.badge && (
                ...
                <tr key={key} className="border-t">
                  ...
                  {plans.map(plan => (
                    <td key={`${plan.id}-${key}`} className="text-center p-2">
                      {typeof plan[key] === 'number' ? (
                        <span className="text-sm font-semibold">{plan[key]}</span>
                      ) : typeof plan[key] === 'boolean' ? (
                        plan[key] ? <CheckCircle className="w-4 h-4 text-green-600 mx-auto" /> :
                ...
                <tr key={feature} className="border-t">
                  <td className="p-2 font-medium">
                    <div className="flex items-center gap-1">
                      ...
                      <InfoTooltip content={infoRegistry[feature]?.description || `More about ${feature}`} />
                    </div>
                  </td>
                  {plans.map((plan) => (
                    <td key={`${plan.id}-${feature}`} className="text-center p-2">
