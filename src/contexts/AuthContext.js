...
export function AuthProvider({ children }) {
  ...
  // ✅ Modified login: uses response body; no /me call here
  const login = useCallback(
    async (email, password, remember = true) => {
      const res = await fetch(LOGIN_ENDPOINT, {
        method: "POST",
      ...
      const roles = deriveRawRoles(data?.user || {});
      setUser({ ...(data?.user || {}), account_type: roles });

      // Return everything so the caller can persist to Redux/localStorage
      return { user: data?.user || null, token: data?.token || null, onboarding: data?.onboarding || null };
    },
    []
  );
