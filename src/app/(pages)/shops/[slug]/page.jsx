...
// A sample categories array used for demonstration.
const categories = ["Electronics", "Fashion", "Homeware"];

// Naively determine the category based on the product name.
...
// Component to render each product in the search suggestion dropdown.
const SearchResultItem = ({ product }) => (
    <Link
        href={`${product?.seller_country || 'gh'}/${product.slug}`}>
...
export default function Shops({ params }) {
  ...
  // Fetch main products for the grid (independent of search).
  useEffect(() => {
    const fetchMainProducts = async () => {
      setMainLoading(true);
      setMainError(null);
      try {
        const response = await fetch(`https://media.upfrica.com/api/shops/${slug}/products/`, {
          method: 'GET',
          redirect: 'follow',
        });
        if (!response.ok) {
  ...
  // Fetch products for search suggestions (independent of main grid).
  useEffect(() => {
    if (!searchQuery.trim()) {
      setSearchResults([]);
    ...
    const timer = setTimeout(() => {
      const fetchSearchResults = async () => {
        ...
        try {
          const endpoint = `https://media.upfrica.com/api/shops/${slug}/products/filter/?q=${encodeURIComponent(searchQuery)}`;
          ...
          // Consider setting an error state for search results if needed.
        } finally {
          setSearchLoading(false);
        }
      };
