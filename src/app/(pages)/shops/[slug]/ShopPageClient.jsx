...
// import { AiOutlineLeft, AiOutlineRight, AiOutlineFilter } from 'react-icons/ai';
// import { HiOutlineCalendar } from 'react-icons/hi';
// import Link from 'next/link';
...
export default function ShopPageClient({ slug }) {
    ...
    useEffect(() => {
        ...
        const fetchFiltered = async () => {
            ...
            const data = res.ok ? await res.json() : { results: [], count: 0 };
    ...
    // Choose which set of products to display
    const displayProducts =
        filterLoading || filterProducts.length > 0 ? filterProducts : mainProducts;

    // Sidebar toggles & outside-click handling
          ...
          /> */}
            {shop.shop_logo ? (
                <img
                    src={shop.shop_logo}
                    alt="Shop Logo"
                    className="h-16 w-16 md:h-20 md:w-20 rounded-full border-2 border-white shadow object-center"
                />
            ) : (
                <div className="h-16 w-16 md:h-20 md:w-20 rounded-full bg-gray-100 flex items-center justify-center text-gray-500">
                    N/A
                </div>
            )}
<div>
