...
function fixImageUrl(u) {
  ...
  // Media-ish relative paths → prefix with API base
  const api = API_BASE.replace(/\/$/, "");
  if (/^\/?media\//i.test(s)) return `${api}/${s.replace(/^\//, "")}`;
  if (/^(direct_uploads|uploads|active_storage|attachments)\//i.test(s)) {
    return `${api}/media/${s}`;
...
function Pill({ children, color = "gray" }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${map[color] || map.gray
        }`}
    >
      {children}
    </span>
...
export default function SalesPage() {
  ...
  async function fetchPage(url, { append = false } = {}) {
    ...
    try {
      const res = await fetch(url, {
        method: "GET",
        credentials: "include", // ensure cookies go with the request in prod
        headers: {
          ...(token ? { Authorization: `Token ${token}` } : {}),
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) msg = "Not authorized. Please sign in again.";
        if (res.status === 403) msg = "Forbidden. Your account can’t access this resource.";
        throw new Error(msg);
      }
  ...
  const undoDispatch = async (order, item) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = res.status === 401 ? "Not authorized." : "Failed to undo dispatch";
        throw new Error(msg);
      }
      // optimistic update
      updateOrderItems(order.id, (it) =>
  ...
  const handleDispatchSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(res.status === 401 ? "Not authorized." : "Failed to dispatch");
