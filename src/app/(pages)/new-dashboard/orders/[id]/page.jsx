    ...
    <div className="flex flex-col sm:flex-row gap-2 sm:gap-2 w-full sm:w-auto">
      ...
      <DeliveryTracker
        ...
        steps={[
          ...
          { label: "Delivered", note: item.receive_status === 1 ? "Delivered" : "Not confirmed" },
        ]}
      />

      {/* Tracking */}
      <div className="mt-4 text-sm">
        <p><strong>Tracking details</strong> &nbsp; {order.tracking_number || "N/A"}</p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
        <div>
          ...
          <div className="flex items-center gap-2 text-gray-700 mt-1">
            <AiOutlineUser />
            <span>{showFullInfo ? `${order.buyer.first_name} ${order.buyer.last_name}` : `${order.buyer.first_name} ${order.buyer.last_name[0]}.`}</span>
          </div>
          <div className="flex items-center gap-2 text-gray-700 mt-1">
            <AiOutlinePhone />
        ...
        <div>
          ...
          <p>Discount &nbsp;&nbsp;&nbsp;&nbsp; -0.00</p>
          <p>Postage &nbsp;&nbsp;&nbsp;&nbsp; 0.00</p>
          <p className="font-semibold">Total &nbsp;&nbsp;&nbsp;&nbsp; GHS {(item.price_cents * item.quantity / 100).toFixed(2)}</p>
        </div>
      </div>
    </div>
  );
