...
export const revalidate = 0; // ensure no static prerender for "/"
...
const languages = Object.fromEntries(MARKETS.map((m) => [m.locale, `${BASE}/${m.cc}`]));
  ...
  return (
    <main className="min-h-[100svh] bg-[#f6f7f9] text-[#111827]">
      <div className="mx-auto max-w-3xl px-4 py-12">
        ...
        <section aria-label="Country picker" className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {MARKETS.map((m) => (
            <Link
              key={m.cc}
              href={`/${m.cc}`}
            ...
            >
              <div className="text-3xl" aria-hidden>{m.flag}</div>
