...
const SearchName = () => {
    ...
    useEffect(() => {
        if (name) {
            const fetchCategoryData = async () => {
                try {
                    const response = await fetch(`https://upfrica-staging.herokuapp.com/api/v1/products?q=${name}`);
    ...
    return (
        <div className={`grid lg:grid-cols-4 gap-10 container mx-auto py-5 ${categoryData.length <= 2 ? 'h-screen' : 'h-auto'}`}>
