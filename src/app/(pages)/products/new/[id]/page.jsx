...
const NewProduct = ({ params }) => {
    ...
    useEffect(() => {
        if (id) {
            const fetchProduct = async () => {
                try {
                    const user = JSON.parse(localStorage.getItem("user")); // LocalStorage থেকে user তথ্য আনুন
                    const token = user?.token; // user থেকে tokenটি আনুন
                    ...
                    myHeaders.append("Authorization", `Bearer ${token}`); // Authorization হেডারে টোকেন যোগ করুন
    
                    const response = await fetch(`https://upfrica-staging.herokuapp.com/api/v1/products/${id}`, {
                        method: 'GET',
                        headers: myHeaders, // Headers-এ Authorization যোগ করুন
                    });
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            fetch("https://upfrica-staging.herokuapp.com/api/v1/products", requestOptions)
